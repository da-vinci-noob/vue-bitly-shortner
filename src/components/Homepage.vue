  ...
  try {
    await axios
      .post(
        ...
        {
          long_url: url.value, //varEmail is a variable which holds the email
          domain: 'bit.ly'
        },
        {
          headers: {
      ...
      .then((response) => (shortUrl.value = response.data))
  } catch (err) {
    console.log(err)
  }
  loading.value = false
...
<template>
  <div class="bg-gray-200 dark:bg-gray-900">
    ...
    <div v-if="loading">
      <div class="fixed top-0 left-0 z-10 block w-full h-full bg-white">
        <span class="flex items-center justify-center h-screen">
          <svg
            class="w-32 h-32 text-green-500 fill-current animate-spin"
    ...
    <div
      v-if="shortUrl"
      class="fixed top-0 left-0 z-10 block w-full h-full bg-white dark:bg-gray-600"
    >
      <div class="flex items-center justify-center h-screen">
        <span
          class="px-6 py-3 bg-red-400 rounded-lg rounded-r-none ring-4 ring-blue-600"
        >
          {{ shortUrl['link'] }}
        </span>
        <span
          @click="reset"
          class="px-6 py-3 text-gray-300 bg-blue-700 rounded-lg rounded-l-none cursor-pointer ring-4 ring-blue-600 hover:animate-pulse"
        >
          Cancel
        </span>
      </div>
